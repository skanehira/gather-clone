...
const ToolButton:React.FC<ToolButtonProps> = ({ children, selected, onClick, className, label, disabled }) => {
    ...
    return (
        <div className='relative' onMouseEnter={() => handleShowToolTip(true)} onMouseLeave={() => handleShowToolTip(false)}>
            <button className={`${selected ? 'bg-light-secondary' : ''} ${disabled ? 'pointer-events-none text-gray-500 cursor-default' : 'hover:bg-light-secondary'} transition-colors duration-300 ease-in-out aspect-square grid place-items-center rounded-lg p-1 ${className}`} onClick={onClick}>
                {children}
            </button>
