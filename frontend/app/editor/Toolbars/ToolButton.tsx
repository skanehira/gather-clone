...
const ToolButton:React.FC<ToolButtonProps> = ({ children, selected, onClick, className, label, disabled }) => {
    ...
    return (
        <div className='relative' onMouseEnter={() => handleShowToolTip(true)} onMouseLeave={() => handleShowToolTip(false)}>
            <button className={`${selected ? 'bg-lightblue' : ''} ${disabled ? 'pointer-events-none text-gray-500 cursor-default' : 'hover:bg-lightblue'} aspect-square grid place-items-center rounded-lg p-1 ${className}`} onClick={onClick}>
                {children}
            </button>
