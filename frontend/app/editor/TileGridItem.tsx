...
const TileGridItem: React.FC<TileGridItemProps> = ({ sheetName, spriteName, selected, onClick }) => {
    ...
    return (
        <div className={`${spriteName === 'empty' ? 'pointer-events-none' : ''} w-full aspect-square hover:bg-light-secondary cursor-pointer rounded-lg flex flex-col items-center justify-between ${selected ? 'bg-light-secondary' : ''}`} onClick={onClick}>
